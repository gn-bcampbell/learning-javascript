/*
  ...
  The input will come from a textarea inserted into the DOM (see code below to insert the elements), 
...
const camelCase = function (word) {
  const [fw, sw] = word.split('_');
  const [upperSW, restSW] = [sw[0].toUpperCase(), sw.slice(1).toLowerCase()];

  const fullWord = [];
  fullWord.push(fw.toLowerCase() + upperSW + restSW);
