...
// Version 2 with chaining
const improvedAverageHumanAgeCalc = (ages) => {

    return agesCopy = ages.map(age => {
        if (age <= 2) return age * 2;
        else return (age = 16 + age * 4);
    })
        .filter(x => x >= 18)
        .reduce((acc = 0, age, index, array) => acc + age / array.length, 0)
}
...
console.log(`Improved 1: ${improvedAverageHumanAgeCalc([5, 2, 4, 1, 15, 8, 3])}`);
console.log(`Improved 2: ${improvedAverageHumanAgeCalc([16, 6, 10, 5, 6, 1, 4])}`);