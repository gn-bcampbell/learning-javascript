...
3.  Foreach remaining dog,log to the console whether it's an adult ("Dognumber1
...
const checkDogs = function (dogsJulia, dogsKate) {
  const juliaPets = [...dogsJulia];
  const justJuliaDogs = juliaPets.slice(1, -2);
  const allDogs = justJuliaDogs.concat(dogsKate);

  allDogs.forEach((dog, index) => {
    let dogString = dog >= 3 ? 'is an adult' : 'is still a puppy';
    console.log(
      `Dog number ${index + 1} ${dogString} and is ${dog} years old.`
    );
  });

  console.log(justJuliaDogs);
};
